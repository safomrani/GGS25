...
export const Agenda25 = (): JSX.Element => {
      ...
      <section className={`w-full mx-auto font-sans px-2 md:px-4 lg:px-6 xl:px-6 2xl:px-16 relative ${inter.className}`} data-model-id="113:102">
        ...
        <div className="lg:hidden">
          <div className="relative pl-1">
            ...
            <div className="absolute left-3 top-4 bottom-0 w-[2px] bg-[#0ca590]"></div>
            
            {/* Combine all events for mobile timeline */}
            <div className="space-y-3">
              {/* Morning Events */}
              {morningEvents.map((event, index) => (
                <div key={`morning-${index}`} className="relative">
                  {/* Timeline dot */}
                  <div className="absolute left-1 top-2 w-[16px] h-[16px] bg-[#0ca590] rounded-full border-2 border-white shadow-md z-10 flex items-center justify-center">
                  ...
                  <div className="ml-6 pb-1">
                    {/* Time badge */}
                    <div className="mb-2">
                      <div className="inline-flex items-center bg-[#0ca590] text-white px-2.5 py-1 rounded-md text-xs font-bold shadow-sm">
                        {event.time}
                      </div>
                    </div>
                    ...
                    {event.type === "break" ? (
                      <div 
                        className="p-3 rounded-lg shadow-sm border-l-2 border-[#D51317]"
                        style={{ backgroundColor: event.bgColor }}
                      >
                        <p className={`font-semibold text-sm text-white ${interMedium.className}`}>
                          {event.title}
                        </p>
                      </div>
                    ) : event.type === "section-header" ? (
                      <div className="bg-[#f0f9f8] rounded-lg shadow-sm border-l-2 border-[#0ca590] overflow-hidden">
                        <div className="p-3">
                          <h3 className={`font-bold text-[#0ca590] text-base mb-3 ${interBold.className}`}>
                            {event.title}
                          </h3>
                          <div className="space-y-2">
                            {event.sessions?.map((session, sessionIndex) => (
                              <div key={sessionIndex} className="bg-white rounded-md p-2.5 shadow-sm">
                                <div className="flex gap-2">
                                  <div className="flex flex-col gap-0.5 mt-0.5 flex-shrink-0">
                                    {session.dots?.map((dot, dotIndex) => (
                                      <div
                                        key={dotIndex}
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <h4 className={`font-black text-[#255345] text-sm leading-tight mb-1.5 ${interBlack.className}`}>
                                      {session.title}
                                    </h4>
                                    <div className="space-y-0.5 text-xs">
                                      <p className={`text-[#0ca590] ${inter.className}`}>
                                        <span className="font-medium">Room:</span>
                                        <span className="font-bold ml-1">{session.room}</span>
                                      </p>
                                      {session.cohostedBy ? (
                                        <p className={`text-[#f0783c] ${inter.className}`}>
                                          <span className="font-medium">Hosted By:</span>
                                          <span className="ml-1">
                                            <span className="font-bold">{session.cohostedBy[0]}</span>
                                            <span className="font-normal"> and </span>
                                            <span className="font-bold">{session.cohostedBy[1]}</span>
                                          </span>
                                        </p>
                                      ) : (
                                        <p className={`text-[#f0783c] ${inter.className}`}>
                                          <span className="font-medium">Hosted By:</span>
                                          <span className="ml-1 font-bold">{session.hostedBy}</span>
                                        </p>
                                      )}
                                    </div>
                                  </div>
                      ...
                      <div className="bg-[#f0f9f8] p-3 rounded-lg shadow-sm border-l-2 border-[#0ca590]">
                        <h3 className={`font-black text-[#255345] text-sm leading-tight mb-1.5 ${interBlack.className}`}>
                          {event.title}
                        </h3>
                        <div className="space-y-0.5 text-xs">
                          {event.room && (
                          ...
                          {event.host && (
                            <p className={`text-[#f0783c] ${inter.className}`}>
                              <span className="font-medium">Hosted By:</span>
                              <span className="ml-1 font-bold">{event.host}</span>
                            </p>
                          )}
                        </div>
                      </div>
              ...
              {afternoonEvents.map((event, index) => (
                <div key={`afternoon-${index}`} className="relative">
                  {/* Timeline dot */}
                  <div className="absolute left-1 top-2 w-[16px] h-[16px] bg-[#0ca590] rounded-full border-2 border-white shadow-md z-10 flex items-center justify-center">
                    <div className="w-[6px] h-[6px] bg-white rounded-full"></div>
                  </div>
                  ...
                  <div className="ml-6 pb-1">
                    ...
                    {event.type === "break" ? (
                      <div 
                        className="p-3 rounded-lg shadow-sm border-l-2 border-[#0ca590]"
                        style={{ backgroundColor: event.bgColor }}
                      >
                        <p className={`font-semibold text-sm text-white ${interMedium.className}`}>
                          {event.title}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="space-y-0.5 text-xs">
                                ...
                                {event.hostedBy && (
                                  <p className={`text-[#f0783c] ${inter.className}`}>
                                    <span className="font-medium">Hosted By:</span>
                                    <span className="ml-1 font-bold">{event.hostedBy}</span>
                                  </p>
                                )}
                              </div>
                            </div>
              ...
              <div className="relative">
                ...
                <div className="ml-6">
                  <div className="bg-white rounded-lg shadow-sm border-l-2 border-[#0ca590] overflow-hidden">
                    <div className="p-3">
                      <h3 className={`font-bold text-[#0ca590] text-base mb-3 ${interBold.className}`}>
                        Tracks
                      </h3>
                      <div className="space-y-2">
                        {tracks.map((track, index) => (
                          <div key={index} className="flex items-start gap-2 p-2 bg-gray-50 rounded-md">
                            ...
                            />
                            <p className={`font-bold text-[#255345] text-xs leading-tight ${interBold.className}`}>
                              {track.name}
                            </p>
                          </div>
                        ))}
